...
class Cipher(object):
    ...
    def __init__(
        ...
        '''
         using this for implementing structs
         pack(), unpack(), calcsize(), pack_into() and unpack_from(). In all these functions,
          we have to provide the format of the data to be converted into binary. 
          Some of the popular format characters are
         '''
        '''
        i , q , b say us about data-types
        '''
        ...
        '''
        Python struct.pack()
        This function packs a list of values into a String representation of the specified type. 
        The arguments must match the values required by the format exactly. Let’s quickly look at struct pack() example:
        '''

        '''
        Python struct.unpack()
        This function unpacks the packed value into its original representation with the specified format. This 
        function always returns a tuple, even if there is only one element. Let’s quickly look at struct unpack() 
        '''
        ...
        # Cyclic key iterator
        cyclic_key_iter = iter_cycle(iter(key))  # ('XYZ') --> X Y Z X Y Z X Y Z...
        ...
        # leaving one and one making pairs
        P = [
        ...
        dup = [[x for x in box] for box in S_boxes]
        print('********************')
        print(dup)

        # Encryption Part
        # assigning a static function to var
        encrypt = self._encrypt

        # initial zeros in left and right
        L = 0x00000000
        R = 0x00000000

        for i in range(len(P)):
            # encrypting the pairs for sub-keys 8+8=16 rounds
            P[i] = L, R = encrypt(L, R, P, S1, S2, S3, S4, u4_1_pack, u1_4_unpack)
            print("\n")
            print("Encryption of Sub-Keys {}".format(i))
            print(P[i])
        # Save P as a tuple since working with tuples is slightly faster,easier
        self.P = P = tuple(P)
        ...
        for box in S:
            for i in range(0, 256, 2):
                ...
                box[i] = L
                print("Encryption of L Sub-Keys {}".format(i))
                print(box[i])
                box[i + 1] = R
                print("\n")
                print("Encryption of R Sub-Keys {}".format(i))
                print(box[i])
        print("\n")
        # Save S
    ...
    def _encrypt(L, R, P, S1, S2, S3, S4, u4_1_pack, u1_4_unpack):
        for p1, p2 in P[:-1]:
            L ^= p1
            a, b, c, d = u1_4_unpack(u4_1_pack(L))  # applying function to Left
            R ^= (S1[a] + S2[b] ^ S3[c]) + S4[d] & 0xffffffff
            R ^= p2
            a, b, c, d = u1_4_unpack(u4_1_pack(R))  # repeats till 8 rounds [0,1,2,3,4,5,6,7] only for right
    ...
    def encrypt_ecb_cts(self, data):
        ...
        # applying the encryption to extra bytes if present
        yield u4_2_pack(
            *encrypt(plain_L, plain_R, P, S1, S2, S3, S4, u4_1_pack, u1_4_unpack)
        )
