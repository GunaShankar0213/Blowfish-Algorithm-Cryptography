...
# each p is assigned with 32 bits hex ( sub-keys)

# keys are stored in array
key = [ 0x4B7A70E9, 0xB5B32944, 0xDB75092E, 0xC4192623,
        0xAD6EA6B0, 0x49A7DF7D, 0x9CEE60B8, 0x8FEDB266,
        0xECAA8C71, 0x699A17FF, 0x5664526C, 0xC2B19EE1,
        0x193602A5, 0x75094C29]
# therefore 32 x 14 == 448 bit

# initializing S-Boxes
'''
4 Substitution boxes(S-boxes) are needed{S[0]…S[4]} in both encryption aswell as decryption process with each 
S-box having 256 entries{S[i][0]…S[i][255], 0&lei&le4} where each entry is 32-bit.
...
'''
a. Rounds: The encryption consists of 16 rounds with each round(Ri) taking inputs the plainText(P.T.) from 
previous round and corresponding subkey(Pi). The description of each round is as follows:
'''

# Finding new P values
for i in range(0,18):
                p[i] = p[i]^key[i%14]  # using mod to repeat the numbers after 14
for i in range(0,18):
                print('New p[{}] = '.format(i),p[i])
print('Printed After XOR')
...
def main():
    ...
    print('The binary format of the string is : '+res)
    if len(res) == 64:
        print('Having 64 bit')
        res_str = res
    else:
        ...
        print(n)
        x = '0'* n
        print('Final Binary Format 64bit : ' + res + x)
        res_str = res + x
        print(res_str)
    encryption(res_str)
...
def encryption(data):
    print(data)
    L = data >> 32 # left part 32 bit
    print("L:" ,L)
    R = data & 0xffffffff # right part and 32 bit
    print("============================================================",R)
    for i in range(0, 16):
        L = p[i] ^ L
        L1 = func(L)
