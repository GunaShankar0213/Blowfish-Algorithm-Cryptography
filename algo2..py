# 18 subkeys{P[0]…P[17]} are needed in both encryption and decryption process 32-bit
p = [
...
# substitution boxes 4 each 256 bits converted to hex
s = [
    [
...
def encryption(data):
    L = data >> 32
    # initial all inputs are 0
    R = data & 0xffffffff
    for i in range(0, 16):
        # XOR operation p[i] to k[i]
        L = p[i] ^ L
        L1 = func(L)
        R = R ^ func(L1)
        L, R = swap(L, R)
